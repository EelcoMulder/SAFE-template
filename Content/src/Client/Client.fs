...
let update (msg : Msg) (currentModel : Model) : Model * Cmd<Msg> =
    match msg with
    | CallMessageApi ->
        currentModel, loadMessage
    | MessageLoaded  (Ok initialMessage) -> 
        let nextModel = { currentModel with Message = Some initialMessage }
        nextModel, Cmd.none
    | _ -> currentModel, Cmd.none
#endif
#if (application == "counter")
    ...
    Field.div [ Field.IsGrouped ]
        [ 
          Control.div [ ]
                [ Button.a
                    [ Button.Color IsPrimary
                      Button.OnClick (fun _ -> dispatch CallMessageApi) ]
                    [ str "Ping" ] ]             
          Control.div [ ]
            [ Input.text
                [ Input.Disabled true
                  Input.Value (showMessage model) ] ]
        ]
#endif
